...
class OSCServer(object):
    def __init__(self, in_port:int=8000, out_port:int=8000, ip='127.0.0.1', osc_attributes=[], model_folder=Path('./models/shortres_msd'),*args) -> None:
        """Initialize server
    ...
    def predict_callback(self, addr, args=[]):
        ...
        # Keep only first tag and send it to /result/ adress
        top_tag = [k  for k, _ in sorted(result_dict.items(), key=lambda item: item[1], reverse=True)[:1]][0]
        self.send(address=f"/result", content=f"{top_tag}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_folder", type=PosixPath, default=Path('./models/shortres_msd'), help="Path to model folder")
